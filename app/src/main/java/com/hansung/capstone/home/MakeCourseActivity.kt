...
class MakeCourseActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    fun moveAndDrawMap() {
            ...
            .enqueue(object : Callback<ResultSearchDirections> {
                override fun onResponse(
                    ...
                    if (body != null) {
                        ...
                        val deco: List<LatLng> = DataConverter.decode(coordinates) // 경로 인코드 값 디코드해서 리스트로 저장
                        pathOverlay.coords = deco
                        pathOverlay.outlineWidth = 0
                        pathOverlay.width = 12
                        pathOverlay.color = Color.parseColor("#87D5AA") // 연두색
        ...
        // 마커 찍기
        for (m in 0 until waypointsPositions.size) {
            if(m == 0 || m==waypointsPositions.size-1) {
                val marker = Marker()
                marker.position = waypointsPositions[m]
                marker.icon = MarkerIcons.BLACK
                marker.iconTintColor = Color.parseColor("#EAE86F") // HoneySuckle
                marker.map = nMap
                markers.add(marker)
            }
            else{
                ...
                marker.position = waypointsPositions[m]
                marker.icon = MarkerIcons.BLACK
                marker.iconTintColor = Color.parseColor("#90EE90") // LightGreen
                marker.map = nMap
                markers.add(marker)
            }
        }
